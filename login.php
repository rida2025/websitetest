...
if( isset($_SESSION['user_id']))
{
  ...
  if( $count > 0 ){
	...
	if(isset($_POST['submit']))
	{
		...
		{
			//read from database
      $stmt = $conn->prepare("SELECT wid, wname, wpassword from wuser where wname = :name");
        $stmt->bindValue(':name', $wname, PDO::PARAM_STR);
        $stmt->execute();
        if ($stmt->rowCount() == 1)
        {
            $result = $stmt->fetch();
            $userid = $result['wid'];
            if(password_verify($wpassword, $result['wpassword'])){
            if( isset($_POST['rememberme']) ){
              // Set cookie variables
              $days = 30;
              $value = encrypt_decrypt('encrypt',$userid);
...
<head>
  ...
  <title>Login - Rush To Learn</title>
            ...
            <div class="col-lg-4 col-md-6 d-flex flex-column align-items-center justify-content-center">
              ...
              <div class="card mb-3">
                ...
                <div class="card-body">
                  ...
                  <form method="post" name="login" class="row g-3 needs-validation" novalidate>

                    <div class="col-12">
                      <label for="yourUsername" class="form-label">Username</label>
                      <div class="input-group has-validation">
                        <input type="text" name="wname" class="form-control" id="yourUsername" required>
              ...
              <div class="credits">
                ...
                Designed and created by <a href="#">Mohammed Reda</a>
              </div>
